...
export class MovieRatingService {
  ...
  getratings() {
    return this.http.get<any>(this.laravelUrl + "/api/movies");      //Funzione che chiama tutti i valori dei campi di interfaccia DataInterface in movie-ratings.model.ts
  }



  getratingsByUserId(user_id:  MovieRatingsArrayInterface) {
    return this.http.get<any>(this.laravelUrl + "/api/movies/user_id/" + user_id);
  }
  ...
  addMovieRating = (data: MovieRatingsArrayInterface) => {
    return this.http.post<MovieRatingsArrayInterface>(this.laravelUrl + '/api/movies', {
